...
def normalized2_path(base_name):
    return os.path.join(NORMALIZED2_DIR, base_name + ".wav")
...
def normalize2(base_name):
    s = librosa.load(sound_path(base_name))
    librosa.output.write_wav(normalized2_path(base_name), s[0]/np.std(s[0]), s[1])
